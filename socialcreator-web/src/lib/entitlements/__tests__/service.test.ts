...
const createMockRepository = (overrides: Partial<IEntitlementRepository> = {}): IEntitlementRepository => ({
  ...
  getCurrentPeriodUsage: vi.fn().mockResolvedValue({ used: 0, periodStart: new Date(), periodEnd: new Date() }),
...
describe("FeatureGateService", () => {
  ...
  describe("consume", () => {
    it("should successfully consume when under limit", async () => {
      mockRepo.getCurrentPeriodUsage = vi.fn()
  ...
  describe("race condition handling", () => {
    it("should handle concurrent consume requests", async () => {
      ...
      mockRepo.getCurrentPeriodUsage = vi.fn().mockImplementation(() => {
        return Promise.resolve({ used: usageCount, periodStart: new Date(), periodEnd: new Date() })
      })
  ...
  describe("monthly quota reset", () => {
    it("should create new period when current period is expired", async () => {
      ...
      mockRepo.consumeUsage = vi.fn().mockImplementation((orgId, featureKey, amount, newPeriodStart, newPeriodEnd) => {
