...
export function VideoCard({ playbackId, duration, status, createdAt, className, onClick }: VideoCardProps) {
  const statusColor = STATUS_COLORS[status] || STATUS_COLORS.UPLOADING;
  const statusLabel = STATUS_LABELS[status] || status;

  return (
    <div onClick={onClick} className={cn("relative bg-surface-card rounded-xl overflow-hidden border border-hairline cursor-pointer transition-all duration-200 hover:shadow-card", className)}>
      <div className="relative aspect-video bg-surface-strong flex items-center justify-center">
        <div className="w-12 h-12 text-muted-soft">
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity bg-ink/20">
          <div className="w-14 h-14 rounded-full bg-surface-card/90 flex items-center justify-center">
            <svg className="w-6 h-6 text-ink ml-1" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
          </div>
        </div>
        {duration && (
      ...
      <div className="p-3">
        <div className="flex items-center justify-between">
          <span className={cn("px-2 py-0.5 rounded-pill text-caption-uppercase text-xs font-semibold", statusColor)}>{statusLabel}</span>
