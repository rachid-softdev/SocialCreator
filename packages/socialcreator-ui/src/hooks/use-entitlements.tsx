...
export function useLimit(limitKey: string): LimitHookResult {
  ...
  return { limit, used, remaining: limit === null ? null : Math.max(0, limit - used), resetAt, isLoading, error };
}
...
interface FeatureGuardProps { feature: string; fallback?: React.ReactNode; children: React.ReactNode; }
...
interface LimitIndicatorProps { feature: string; showBar?: boolean; }
...
export function LimitIndicator({ feature, showBar = true }: LimitIndicatorProps) {
  ...
  return (
    <div className="flex flex-col gap-1">
      <span className={`text-sm ${percentage >= 100 ? "text-red-600" : percentage >= 80 ? "text-yellow-600" : "text-gray-600"}`}>
        {used} / {limit} ({remaining} remaining)
      </span>
      {showBar && (
        <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
          <div className={`h-full transition-all ${percentage >= 100 ? "bg-red-500" : percentage >= 80 ? "bg-yellow-500" : "bg-green-500"}`} style={{ width: `${Math.min(percentage, 100)}%` }} />
        </div>
      )}
      {resetAt && <span className="text-xs text-gray-400">Resets: {new Date(resetAt).toLocaleDateString()}</span>}
    </div>
  );
}

interface UpgradeBannerProps { feature?: string; message?: string; }

export function UpgradeBanner({ feature, message = "Upgrade your plan to access this feature" }: UpgradeBannerProps) {
  return (
    <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-4">
      <div className="flex items-center gap-3">
        ...
        <a href="/settings/billing?upgrade=true" className="flex-shrink-0 px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors">Upgrade</a>
      </div>
    </div>
  );
}

export default { useEntitlements, useFeature, useLimit, useCanConsume, FeatureGuard, LimitIndicator, UpgradeBanner };