# QA end-to-end avec Playwright (SocialCreator)
...
Start-Process -FilePath cmd -ArgumentList "/c","pnpm dev (depuis socialcreator-web)" 
  -RedirectStandardOutput "$env:TEMP\qa\SocialCreator.dev.txt" 
  -RedirectStandardError "$env:TEMP\qa\SocialCreator.err.txt" -WindowStyle Hidden
...
- GET / â†’ **HTTP 200**.
GET / renvoie HTTP 200 avec le titre SocialCreator - AI-Powered Social Media Content. Racine publique, aucun mur d auth sur /.
...
Aucun bypass d auth dev trouve dans socialcreator-web. Pages publiques testables ; pages protegees redirigent vers /login. Ajouter un bypass dev ou identifiants de seed pour les tester.
...
`
socialcreator-web/scripts/qa\        # (Ã  crÃ©er si campagne complÃ¨te)
